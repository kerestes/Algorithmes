...
public class Joueur {
    ...
    private int base;
    private Node[][] nodes;
    private AtomicInteger countColoring = new AtomicInteger(0);
    private AtomicInteger countBack = new AtomicInteger(0);
    ...
    public Node[][] jouer(){
        Node[][] returnNodes = graphColoringAndBackTracking(cloneArrayNodes(nodes), null);
        System.out.println("GraphColoring iteration: " + countColoring.get());
        System.out.println("BackTracking Iteration: " + countBack.get());
        return returnNodes;
    }
    ...
    private Node[][] graphColoringAndBackTracking(Node[][] copyNodes, List<Node> listNodePossibilities){
        ...
        while (true){
            ...
            //System.out.println(print(copyNodes));
    ...
    private String print(Node[][] copyNode){
        String line = "";
        for (int i=0; i<base*base; i++){
            line += "| ";
            for(int j=0; j<base*base; j++){
                if(copyNode[i][j].getValue() == 0){
                    line += "   | ";
                } else{
                    line += copyNode[i][j].getValue() < 10 ? "0" + copyNode[i][j].getValue() : copyNode[i][j].getValue();
                    line += " | ";
                }
            }
            line += "\n";
