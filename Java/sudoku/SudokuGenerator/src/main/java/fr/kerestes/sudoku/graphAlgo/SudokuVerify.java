...
public class SudokuVerify {
    ...
    private boolean thereIsOneResult= false;
    private List<Node> result;

    public SudokuVerify(){
        result = new ArrayList<>();
    }
    public void insertTable(ColumnHead columnHead, Integer[][] board, int base){
        this.base = base;
        this.size = base*base;
        columnHeadRoot = columnHead;
        this.thereIsOneResult = false;
        for(int row=0; row<size; row++){
            for(int column=0; column<size; column++){
                if(board[row][column] != 0){
                    int verify = result.size()+1;
                    columnHead = columnHeadRoot;
    ...
    public boolean search(){
        while (columnHeadRoot != columnHeadRoot.rightNode || !thereIsOneResult){
            ...
            if( columnHead == columnHead.downNode || columnHeadRoot == columnHeadRoot.rightNode){
                ...
                if(node.head.choice == Integer.MAX_VALUE && thereIsOneResult)
                    return true;
