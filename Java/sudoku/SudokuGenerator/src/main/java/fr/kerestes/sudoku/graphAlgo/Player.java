...
public class Player {
    ...
    private List<Node> result1;
    private List<Node> result;

    private boolean verifyOtherPossibilities = false;
    ...
    private boolean findAnswer(){
        while (columnHeadRoot != columnHeadRoot.getRightNode() || !verifyOtherPossibilities){
            if(columnHeadRoot == columnHeadRoot.getRightNode()){
                verifyOtherPossibilities = true;
                result1 = new ArrayList<>(result);
            }
            ColumnHead columnHead = findLowestSize();
            Node node = columnHead;

            if(columnHead == columnHead.getDownNode() ||  (columnHeadRoot == columnHeadRoot.getRightNode() && verifyOtherPossibilities) ){
