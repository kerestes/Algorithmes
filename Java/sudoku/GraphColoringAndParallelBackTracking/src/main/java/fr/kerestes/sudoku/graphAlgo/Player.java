...
public class Player extends RecursiveTask<Node[][]> {

    int base;
    Node[][] nodes;
    List<Node> listNodePossibilities;

    //The difference between this algorithm and the GraphColoringAndBacktracking algorithm
    // is that it uses the RecursiveTask interface to process the possibilities in parallel
    ...
    public Player(int base, Node[][] nodes, List<Node> newListNodePossibilities){
    ...
    private Node[][] backTracking(Node[][] copyNodes, List<Node> listNodePossibilities){
        ...
        List<Player> tempListJouer = new ArrayList<>();
        for(int k=0; k<copyNodes[currentNode.getLine()][currentNode.getColumn()].getPossibilitiesSize(); k++ ){
            ...
            tempListJouer.add(new Player(base, copyNodes2, newListNodePossibilities));
        }
